...
public:
    BayerToH264Converter2(unsigned int device_num, unsigned int input_width, unsigned int input_height):
        ...
        sws_ctx_(nullptr) 
    {
            pkts_.resize(device_num_, nullptr);
            frameIndexes_.resize(device_num_, 0);
            results_.resize(device_num_, false);
    ...
    ~BayerToH264Converter2() 
    {
        ...
        for (unsigned int i = 0 ; i < device_num_; i++)
        {
    ...
    bool initializeContexts(const char* outputFileName) 
    {
        ...
        for (unsigned int i = 0 ; i < device_num_ ; i++)
        {
            ...
            codec_contexts_.back()->time_base = {1,21} ;
            codec_contexts_.back()->framerate = {21,1}; 
    ...
    {
         assert( n_curr_camera < device_num_  && "# of Current Camera must not be less than device number");
