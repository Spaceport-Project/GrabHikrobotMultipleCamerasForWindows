...
public:
    ...
    bool writeAllFrames2MP43()
    {
        ...
        while (!m_bExit || !queuevecPkts_.empty()) 
        {
            std::vector<AVPacket*> packets={nullptr};
            {
                ...
                std::unique_lock<std::mutex> lk(writeMutex_);
                writeCond_.wait(lk, [this] {return  !queuevecPkts_.empty();});
                
                packets = queuevecPkts_.top();
                queuevecPkts_.pop();
                

                if (!m_bExit) std::this_thread::sleep_for(std::chrono::milliseconds(100));
                std::cout<<"size:"<<queuevecPkts_.size()<<std::endl;
